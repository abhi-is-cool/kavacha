...
Planned next entries (port milestone M2): `0003-build-wire-kavacha-component`
(`browser/components/moz.build` `DIRS`), `0004-browser-xhtml-kavacha-include` (one
`<script>` + one `<link>`), `0005-preferences-kavacha-panes` (`preferences.xhtml`/`.js`/`jar.mn`),
`0006-search-config-brave-default` (`services/settings/dumps/main/search-config-v2.json`).

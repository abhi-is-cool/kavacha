...
| **R8** | **Windows native build** | v1.0 | **Unblocked 2026-09-19** by [ADR 0020](decisions/0020-firefox-esr-direct-overlay.md). That ADR originally attributed the `xul.dll` link failure to Zen's libwebrtc config; **both halves of that were wrong and it is amended twice** — the same failure reproduced on vanilla Firefox ESR 153, and the real cause is **MAX_PATH**: make hands the link prerequisite to the Win32 API with its `..` hops un-normalised, which measured 262 characters against a 260 limit on a GitHub runner. Fixed 2026-09-21 by `KV_OBJDIR=D:/o` ([VERIFICATION.md](VERIFICATION.md) §4j). Closes when CI publishes a Windows installer to the `nightly` release (port milestone M4). **Observed on the runner 2026-09-21:** build, `kavacha-153.4.0.en-US.win64.installer.exe` and an uploaded artifact — progress, not closure; the installer has not been **published** to the `nightly` release, which is what R8 actually requires. |

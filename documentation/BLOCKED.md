...
| # | Item | Why it is blocked | What unblocks it |
|---|---|---|---|
| **B4** | **Windows native build** | ~~Upstream-broken at the Zen pin (libwebrtc rule missing when linking `xul.dll`; Zen only cross-compiles Windows).~~ **Decided 2026-09-19 ([ADR 0020](decisions/0020-firefox-esr-direct-overlay.md)): the Zen base is retired; Kavacha overlays Firefox ESR directly, where native Windows is a tier-1 path.** No longer blocked on a decision — it is ordinary work: a native build on a Windows host (this box), then a CI leg. **M1 observed 2026-09-19** (VERIFICATION §4e: build, installer, launch, R3 pass on this host). **Regressed in CI 2026-09-20:** the first `windows-latest` run failed after 179 min with the *same* `no rule to make target … Unified_cpp_…gn0.obj` / `xul.dll` failure this row once attributed to Zen — on vanilla Firefox ESR 153, no Zen in the tree. ADR 0020 reason 2 is amended accordingly. The cause is not the source, pin, patches or mozconfig (all reproduced correctly on this host, including a configure run with CI's own options) and is **not yet diagnosed**; macOS and Linux built green in the same run. Stays listed until CI publishes a Windows asset (M4). | Nothing from you. Evidence: a green CI run. |

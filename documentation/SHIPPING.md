...
| **R3** | **Network-silence test in CI** | Beta / Phase 4 | **Test written and validated locally (2026-08-07)** — `test/network-silence/`. Runs any built `Kavacha.app` via MOZ_LOG DNS capture and fails on any telemetry/ads/experimentation host. **Local result: PASSES** — 0 such hosts on a fresh idle profile. It also surfaced a fresh-profile phone-home nothing else caught: the **GMP media-plugin update ballot to `aus5.mozilla.org`** (plus Widevine→Google, OpenH264→Cisco, Web Push→Mozilla, Google connectivity check), which patch 0002 did not silence (`media.gmp-manager.url` untouched). Silencing those is a product decision (codec/DRM/notification tradeoff) — flagged, not changed. **Still open: CI wiring** — needs the hosting decision (nightly-runner step vs self-hosted job); see the test's README. |

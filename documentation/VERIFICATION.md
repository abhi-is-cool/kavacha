...
| **D5** | Theme `kavacha-midnight` leaves `--kavacha-accent` unset. **Adjudicated 2026-08-01: INTENTIONAL, not a bug.** `ui/defaults/kavacha-ux.js` records the 2026-07-13 decision that picking an accent "is the user's", ships no default accent, and the welcome flow asks for one; Forest sets `#E0A458` because a *bundled* theme may express taste, the *default* may not. The residual defect is that consumers fall back **silently** — the fix is a documented fallback token, not an accent value in Midnight. | 0023 L4 probe, 2026-08-01 | 🟡 reclassified — token contract, not a colour |

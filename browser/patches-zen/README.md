> **Retired series (2026-09-19).** These 87 patches targeted Zen Browser at `425f0ae1` and are
> **not applied** by the build any more. Kavacha now overlays Firefox ESR directly
> ([ADR 0020](../../documentation/decisions/0020-firefox-esr-direct-overlay.md)); the live series
> is in [`../patches/`](../patches/README.md). This directory is read-only reference for the port
> and is deleted at parity (milestone M3).
...
| `0060-menu-panel-declutter.patch` | **The ⚙ menu was a list, not a menu.** 0030's registry-generated panel rendered flat — 22 command rows + 6 headers + Settings, ~720px in a 758px window, which is what 0037 and 0040 were both patching around. Settings stays pinned; each domain collapses to one row carrying its command count, accordion-style; a filter field searches every domain at once. Nothing removed: resting height 291px, and every section now fits without scrolling |
| `0061-theme-engine-correctness.patch` | **The default theme took a different code path from every other theme**, returning early and leaving 12 of 18 tokens undefined, so tab and menu colours fell back to a *text*-derived mix and changed character when you switched away and back. Also: three definitions of "Midnight" of which the winner was the one nobody edited; `color-scheme` written inline on `:root` only, so a light theme left popups, menus and content dark; two light/dark derivations that could disagree; a colour parser that answered "dark" for `hsl()`/`oklch()`/named colours, silently. Adds a real WCAG contrast check that repairs an unreadable theme instead of shipping it |
| `0062-theme-aware-content-pages.patch` | **about:studio, about:marketplace, about:plugins and the new tab never followed the theme** — content documents cannot see the chrome token floor, so every `var(--kavacha-*)` resolved to its dark literal and all four pinned `color-scheme: dark`. Pick Daylight, get a light browser around four black pages. A shared privileged script stamps the live tokens, mode and accent onto each document root; deferred by 0046 as needing a full build, which it turned out not to |
| `0063-appearance-panel.patch` | **Appearance had no home and showed no colour.** It was split across the ⚙ menu's nine-row Appearance section, two different Settings panes, about:studio and Zen's Look and Feel; the accent picker was two clicks deep and themes were names you could not see. One panel now holds Dark/Light/Auto, theme cards painted from each theme's own palette (badged when contrast fails), the accent swatches, and tab/density/sidebar — with the rest of the Appearance domain still generated from the registry. Also fixes the accent reset that looked like it did nothing, a theme menulist that could duplicate or blank, and Arc being unreachable outside about:studio |

| `0064-theme-mode-single-authority.patch` | **"Where are my tabs?"** — they were painted white on white. 0061 made CSS agree with the theme mode and it does, but `--toolbox-textcolor` is not CSS: ZenGradientGenerator writes it INLINE from a dark/light getter that reads `zen.view.window.scheme`, whose default means "always dark". Under Daylight all 21 text-bearing elements in the sidebar sat at 1.04:1 against their own surface. Pins the token to the theme's own text colour with the one declaration form that outranks an inline style, and aligns Zen's pref so its accent derivation stops running dark-mode math on a light theme. Also teaches the colour parser `color(srgb …)` — the syntax `color-mix()` results come back in, which returned null and silently left the mode unchanged — and stops the sidebar-width rule from overriding Zen's splitter |
...
*Rows `0082`–`0087` below were **built and L4-verified on this Zen base on 2026-08-27**
(`build/marionette-phase7.py`, 75/75, Apple Silicon — commit `dd49da9`). That first run
found three defects no static gate caught: `KavachaKnowledgeGraph.sys.mjs` was created but
never registered in `EXTRA_JS_MODULES` and so never packaged; its entity parser dropped
every one-character name; and focus mode left notifications denied because the park pref's
default was pruned by libpref. All three were fixed in `0083`/`0084` here and re-probed
clean. They were later ported to the Firefox ESR base and run again there on 2026-09-20
(77 checks, VERIFICATION.md §4h) — a second run on a different base, not a first run.
Before 2026-08-27 these six had been through no completed build at all: two attempts were
made and neither finished, the second for lack of memory on the development machine. See

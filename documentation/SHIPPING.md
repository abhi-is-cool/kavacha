...
| Windows / macOS / Linux builds | ✅ **All three on the Firefox ESR base, published by CI 2026-09-22** (run 206, `b2c7aac`): `kavacha-nightly-linux-x86_64.tar.xz`, `kavacha-nightly-macos-arm64.dmg`, `kavacha-nightly-windows-x86_64.exe` + `.zip`, no carried-forward warning ([VERIFICATION.md](VERIFICATION.md) §4k). Unsigned — R2 still open. |
...
| **R3** | **Network-silence test in CI** | Beta / Phase 4 | **Test written and validated locally (2026-08-07)** — `test/network-silence/`. Runs any built `Kavacha.app` via MOZ_LOG DNS capture and fails on any telemetry/ads/experimentation host. **Local result: PASSES** — 0 such hosts on a fresh idle profile. It also surfaced fresh-profile phone-homes nothing else caught. **Resolved 2026-08-08** (user decision): the GMP update ballot to `aus5.mozilla.org` is silenced (`media.gmp-manager.url`), Widevine/DRM is off (`media.eme.enabled` + `media.gmp-widevinecdm.*`), and Web Push is off (`dom.push.enabled`) — all privacy-first but **unlocked** so a user can re-enable any of them (DRM via Settings' own checkbox). OpenH264 (WebRTC codec) is deliberately kept. Re-verified: those hosts gone, R3 still passes. **CI: wired** — a `Network-silence test (R3)` step runs the built binary headless on the nightly Linux runner after packaging. |
...
| **R8** | **Windows native build** | v1.0 | ✅ **CLOSED 2026-09-22.** CI published `kavacha-nightly-windows-x86_64.exe` (86,160,093 bytes) to the `nightly` release on a green scheduled run ([VERIFICATION.md](VERIFICATION.md) §4k). Worth remembering how: [ADR 0020](decisions/0020-firefox-esr-direct-overlay.md) blamed the `xul.dll` link failure on Zen's libwebrtc config; that was falsified when the same failure appeared on vanilla Firefox ESR 153, and the real cause was **MAX_PATH** — 262 characters against a 260 limit, because make hands the link prerequisite to the Win32 API with its `..` hops un-normalised. Fixed by `KV_OBJDIR=D:/o`. |
